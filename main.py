...
# 3. إعدادات تخطي حماية سناب شات (تخطي حاجز الـ 3 مقاطع)
def get_ydl_options():
...
# 4. الحفاظ على تشغيل السيرفر في Render
def handle_render():
    server_address = ('', 8080)
...
def send_welcome(message):
    bot.reply_to(message, "🚀 تم تفعيل نظام البروكسي الدوار.\nأرسل يوزر السناب الآن لسحب القصة كاملة.")
...
def handle_all_messages(message):
    ...
    wait_msg = bot.reply_to(message, "🔍 جاري فحص القصة وتخطي الحماية... لحظات.")
    ...
    try:
        with yt_dlp.YoutubeDL(get_ydl_options()) as ydl:
            ...
            total = len(entries)
            if total == 0:
                bot.edit_message_text("❌ لم يتم العثور على سنابات عامة.", message.chat.id, wait_msg.message_id)
                return
            ...
            for i, entry in enumerate(entries, 1):
                ...
                try:
                    # فحص إذا كان الرابط لصورة أو فيديو
                    if any(ext in m_url.lower() for ext in [".jpg", ".png", ".jpeg"]):
                        bot.send_photo(message.chat.id, m_url, caption=f"سنابة رقم [{i}]")
                    else:
                        bot.send_video(message.chat.id, m_url, caption=f"سنابة رقم [{i}]")
                    
                    time.sleep(1.5) # تأخير لتجنب الحظر من تلجرام
                except Exception:
                    continue
        ...
        bot.edit_message_text("⚠️ فشل الاتصال. حاول مرة أخرى (قد يكون البروكسي مضغوطاً).", message.chat.id, wait_msg.message_id)

if __name__ == "__main__":
    # حل مشكلة Conflict 409 الظاهرة في سجلاتك
    try:
        bot.remove_webhook()
    except:
        pass
